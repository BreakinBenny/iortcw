...
static void PM_FlyMove( void ) {
	...
	if ( pm->ps->aiChar == AICHAR_NONE || pml.ladder )
		...
		scale = 1.0;	// AI is allowed to fly freely
	//
	// user intentions
	//
	if ( !scale ) {
...
//----(SA)	end
		...
		if(delta > 38.75) {
			// check that player's alive before playing pain grunts
...
static void PM_Footsteps( void ) {
	...
	if ( animGap > 0 ) {
		...
		if ( pm->ps->aiChar == AICHAR_SUPERSOLDIER || pm->ps->aiChar == AICHAR_PROTOSOLDIER )	//iswalking = qfalse;
...
static void PM_BeginWeaponChange( int oldweapon, int newweapon, qboolean reload ) {  //----(SA)	modified to play 1st person alt-mode transition animations.
	...
	if ( oldweapon == WP_GRENADE_LAUNCHER || oldweapon == WP_GRENADE_PINEAPPLE || oldweapon == WP_DYNAMITE || oldweapon == WP_PANZERFAUST ) {
...
int PM_WeaponClipEmpty( int wp ) {
	if ( pm->noWeapClips ) {
		if (!(pm->ps->ammo[BG_FindAmmoForWeapon(wp)])) || (!(pm->ps->ammoclip[BG_FindClipForWeapon(wp)]))
			return 1;
	}
...
// jpw
	...
	if ( pm->waterlevel == 3 ) {
		if ( pm->ps->weapon != WP_KNIFE && pm->ps->weapon != WP_GRENADE_LAUNCHER && pm->ps->weapon != WP_GRENADE_PINEAPPLE ) {
	...
	switch ( pm->ps->weapon ) {
	default:
		if ( !weaponstateFiring )	// delay so the weapon can get up into position before firing (and showing the flash)
	...
	case WP_FG42SCOPE:
		if ( !weaponstateFiring ) {
			if ( pm->ps->aiChar && pm->ps->weapon == WP_VENOM )	// AI get fast spin-up
				pm->ps->weaponDelay = 150;
			else	// delay so the weapon can get up into position before firing (and showing the flash)
				pm->ps->weaponDelay = ammoTable[pm->ps->weapon].fireDelayTime;
			}
		} else
...
//				PM_AddEvent( EV_SPINUP );
		...
		if ( !delayedFire ) {
			if ( pm->ps->aiChar )	// ai characters go into their regular animation setup
				...
				if ( PM_WeaponAmmoAvailable( pm->ps->weapon ) ) {
					...
					else	// start at four seconds and count down
	...
	if ( pm->ps->weapon ) {
		...
		if ( ammoNeeded > ammoAvailable ) {
			...
			if ((pm->ps->eFlags & EF_MELEE_ACTIVE) || (pm->ps->weapon == WP_SNOOPERSCOPE))	// not going to be allowed to reload if holding a chair
				reloadingW = qfalse;
...
// jpw
	...
	if ( ( pm->ps->aiChar != AICHAR_PROTOSOLDIER ) && ( pm->ps->aiChar != AICHAR_SUPERSOLDIER ) && ( ammoTable[pm->ps->weapon].maxHeat && pm->ps->weapHeat[pm->ps->weapon] ) ) {
		// it is overheating
...
void PmoveSingle( pmove_t *pmove ) {
	...
	if ( !isDummy )	// RF, update conditional values for anim system
	...
	if ( !(pm->ps->pm_flags & PMF_RESPAWNED) && pm->ps->pm_type != PM_INTERMISSION && pm->ps->pm_type != PM_NOCLIP ) {
		...
		if ( !( pm->cmd.wbuttons & WBUTTON_ZOOM ) ) {
			if ( pm->cmd.buttons & BUTTON_ATTACK ) {
				// check for ammo
				if ( PM_WeaponAmmoAvailable( pm->ps->weapon ) )	// all clear, fire!
	...
	if ( pm->ps->powerups[PW_FLIGHT] )	// flight powerup doesn't allow jump and has different friction
		PM_FlyMove();	// RF, removed grapple flag since it's not used
#if 0
